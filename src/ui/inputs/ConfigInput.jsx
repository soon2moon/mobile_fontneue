...
const ConfigInput = ({ icon, label, value, onChange, suffix = "", scaleFactor = 1, ...props }) => {
  ...
  return (
    <div className="flex items-center bg-sunken rounded-md px-2 focus-within:ring-1 focus-within:ring-edge-strong transition-all overflow-hidden h-8 gap-2">
      {icon ? (
        <span className="flex items-center justify-center text-muted w-3.5 shrink-0">
          {icon}
        </span>
      ) : label ? (
        <span className="text-xs font-medium text-muted w-3.5 shrink-0 select-none flex items-center justify-center">
          {label}
        </span>
      ) : null}
      <input
        ...
        className="w-full text-xs text-left bg-transparent border-none outline-none py-1 text-ink font-mono"
        {...props}
      />
    </div>
  );
